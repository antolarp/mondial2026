...
export function calculerPlaces(joueurs, matchs, resultats) {
  ...
  matchsJoues.forEach(match => {
    ...
    const maxPts = cumul[sorted[0].nom]
    const minPts = cumul[sorted[sorted.length - 1].nom]
    sorted.filter(j => cumul[j.nom] === maxPts).forEach(j => premiere[j.nom]++)
    sorted.filter(j => cumul[j.nom] === minPts).forEach(j => derniere[j.nom]++)
  })
...
export function calculerEvolutionClassement(joueurs, matchs, resultats) {
  const matchsJoues = matchs
    .filter(m => resultats[m.id])
    .sort((a, b) => new Date(a.date) - new Date(b.date))
  ...
  return matchsJoues.map((match, idx) => {
    ...
    const point = { match: `J${idx + 1}` }
    joueurs.forEach(joueur => {
      point[joueur.nom] = joueurs.filter(j => cumul[j.nom] > cumul[joueur.nom]).length + 1
    })
    return point
  })
}
